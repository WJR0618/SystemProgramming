...
string format4(string opcode, string r1){
	string n,i,x,b,p,e;
	string objectcode = "";
	string opcodebinary = "";
	n = r1[0] == '@' ? "1" : "0";
		...
		for( int x = 0; x < optab.size(); x++ ){
			if( toUPPER(tokenGroupSplited[i].opcode) == optab[x].ins ){
				...
				if( tokenGroupSplited[i].isFormat4 && isSICXE ){
					output[i].object_code = format4(optab[x].opcode, tokenGroupSplited[i].oprands[0]);
				} //if
...
void printOutput( string fileName ) {
	...
	for(int i = 0; i < output.size(); i++){
		...
		outFile << output[i].line << "\t" << intToA(output[i].loc,16) << "\t";
